...
#test = "This is a sample. I want to convert these? Into a list! How about that?"

def get_average_sentence_length(text):
    all_q_marks = text.replace("?", ".")
    clean_sent = all_q_marks.replace("!", ".")
    sentences = clean_sent.split(".")
    sentence_lengths = []
    all_length = 0
    for sentence in sentences:
        words_in_sentences = sentence.split()
        sentence_lengths.append(len(words_in_sentences))
    for length in sentence_lengths:
        all_length += length 
    average = all_length / len(sentence_lengths)
    return average 
...
class TextSample:
    ...
    def __repr__(self):
        return "The author\'s name is {author} and their average sentence length is {length}.".format(author=self.author, length=self.average_sentence_length)
...
murderer_sample = TextSample(murder_note, "???")
...
def prepare_text(text):
    ...
    no_punct = lower_case.replace(".", "").replace("?", "").replace(",", "").replace("'", "").replace("!", "").replace('"', "")
    prepared_text = no_punct.split()
...
def frequency_comparison(table1, table2):
    ...
    for key in table2.keys():
        if key not in table1.keys():
            appearances += table2[key]
    freq_comp_score = mutual_appearances/appearances
...
def percent_difference(value_1, value_2):
    perc = abs(value_1 - value_2) / ((value_1 + value_2)/2)
    return perc
...
print("Compared to the murder note, Lily's intro had a similarity score of: " + str(find_text_similarity(murderer_sample, lily_sample)))
print("Compared to the murder note, Myrtle's intro had a similarity score of: " + str(find_text_similarity(murderer_sample, myrtle_sample)))
print("Compared to the murder note, Gregg's intro had a similarity score of: " + str(find_text_similarity(murderer_sample, gregg_sample)))
